...
public class DialogViewUser {
    ...
    public void initialize(final ChatList chatlist){
        ...
        Glide.with(context).load(chatlist.getImage()).into(profile);

        btnChat.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                context.startActivity(new Intent(context, ChatActivity.class)
                        .putExtra("phoneID",chatlist.getPhoneID())
                        .putExtra("userName",chatlist.getName())
                        .putExtra("userImage",chatlist.getImage()));
                dialog.dismiss();
            }
        });
...
//                Common.IMAGE_BITMAP = dr.getCurrent().getBitmap();
